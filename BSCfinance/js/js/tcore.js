...
// ============= Duplicate =================
const TCoreAddress = '0xf5128928f85f16bd67c1e02ddd1b47a71d17adf5';
...
const TCoreV2Address = '0xd81b3aa8d4a995c8676524568c387f594297a149';
...
async function connect() {
  ...
  if (!isMetaMaskInstalled()) {
    document.getElementById('warning').innerHTML =
      'MetaMask is not installed, thus you cannot interact with the TCORE Vault. Please install MetaMask from <a href="https://metamask.com/download.html/">here</a> and try again.';
...
async function loadSummary() {
  var TcoreTokenInstance = new web3.eth.Contract(
    JSON.parse(ABI.TcoreToken),
    TCoreAddress
  );
  var lockedLP = web3.utils.fromWei(
    await TcoreTokenInstance.methods.balanceOf(LPTokenAddress).call()
  );
  document.getElementById('Tcore-locked').innerHTML = Number(lockedLP).toFixed(
    5
  );

  var TCoreBalance = web3.utils.fromWei(
    await TcoreTokenInstance.methods.balanceOf(address).call()
  );
  document.getElementById('TCore-wallet-balance').innerHTML = Number(
    TCoreBalance
  ).toFixed(5);
  ...
  var claimable = web3.utils.fromWei(
    await vaultV2.methods.pendingTcore(0, address).call()
  );
  document.getElementById('TCore-claimable-balance').innerHTML = Number(
    claimable
  ).toFixed(5);
}
...
async function claim() {
  var token = new web3.eth.Contract(JSON.parse(ABI.TcoreToken), TCoreAddress);
...
async function loadTVaultData() {
  ...
  var vault = new web3.eth.Contract(
    JSON.parse(ABI.TcoreVault),
    TVaultAddress
  );
  var pendingTcore = await vault.methods.pendingTcore('0', address).call();
  ...
  const Tcore = new web3.eth.Contract(JSON.parse(ABI.ERC20), TCoreAddress);
  ...
  var averageRewards = await vault.methods
    ...
    .call();
  var TCoreLP = await Tcore.methods.balanceOf(LPTokenAddress).call();
  var ethPerLP = wethLP / totalLP;
  ethPerLP = ethPerLP * 2;
  var ethPerTCore = wethLP / TCoreLP;
  var lockedLPValue = web3.utils.fromWei(lockedLP, 'ether') * ethPerLP;
  var TCorePerYear = web3.utils.fromWei(averageRewards, 'ether') * 2103840;
  var ethPerYear = TCorePerYear * ethPerTCore;
  var APY = (ethPerYear / lockedLPValue) * 100;
  APY = APY.toFixed(3);
  // document.getElementById('apyLP-v1').innerHTML = APY
  // var price = wethLP / TCoreLP
  // document.getElementById('priceTcore').innerHTML = "Tcore Price<br>" + price.toFixed(4) + " ETH"
  ...
  var pendingTcore = web3.utils.fromWei(
    await vault.methods.pendingTcore(0, address).call()
  );
...
async function loadTVaultV2Data() {
  ...
  var totalLP = await pair.methods.totalSupply().call();
  var wethLP = await weth.methods.balanceOf(LPTokenAddress).call();
  var lockedLP = await pair.methods.balanceOf(TCoreV2Address).call();
  var averageRewards = await vault.methods
    .averageFeesPerBlockSinceStart()
    .call();
  var TCoreLP = await Tcore.methods.balanceOf(LPTokenAddress).call();
...
async function stakeLPV2() {
  ...
  const allowance = await LPToken.methods
    .allowance(address, TCoreV2Address)
    .call();
...
async function loadTPool() {
  ...
  const tCoreLP = await tcore.methods.balanceOf(LPTokenAddress).call();
  const prices = await lookUpPrices(['ethereum']);
  const tCorePrice = (wethLP / tCoreLP) * prices['ethereum'].usd;

  // take deposit amount, multiple by 0.5%, and mul by korePrice
  const estimateKoreRewards =
    (yourShareUnderlyingAmount * 5) / 1000 / tCorePrice;
  document.getElementById('est-tcore-rewards').innerHTML = toFixed(
    estimateKoreRewards,
    5
  );
  // APY
...
async function approveDaiToTCorePool() {
